...
namespace TplWorkflow.Extensions
{
  ...
  public static class ReflectionExtensions
  {
    ...
    {
      var types = info.GetParameters().Select(p => p.ParameterType).Concat(new[] { info.ReturnType }).ToArray();

      return Delegate.CreateDelegate(Expression.GetDelegateType(types), instance, info);
    }
    ...
    public static Delegate MakeDelegate(this MethodInfo info)
    {
      ...
      return Delegate.CreateDelegate(Expression.GetDelegateType(types), info);
    }
